...
uasort($data, static function($a, $b){
    ...
    $isDirA = is_dir($actualDir . '/' . $a);     //fixed
    $isDirB = is_dir($actualDir . '/' . $b);

    if($isDirA === true && $isDirB === false){
        return -1;
    }
    elseif ($isDirA === true && $isDirB === true){
        return 0;
    }
    else{
        return 1;
...
<ul class="nav nav-tabs">
    ...
    <?php
        $countFor = count($explodedBreadcrumbs);                       //fixed

        for($key = 0; $key < $countFor; $key++):
